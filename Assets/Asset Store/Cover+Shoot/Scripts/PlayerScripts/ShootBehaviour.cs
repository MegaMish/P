...
public class ShootBehaviour : GenericBehaviour
{
	...
	public Vector3 RightHandAim;
	public Vector3 ReloadAim;
		...
		else
		{
			...
			if (Physics.Raycast(ray, out hit, 500f, shotMask))
			{
				if (hit.collider.transform != this.transform)
				{
					...
					DrawShoot(weapons[weapon].gameObject, hit.point, hit.normal, hit.collider.transform, hit.collider.gameObject);
					...
					{
						hit.collider.gameObject.GetComponent<Robot>().TakeDamage(hit.point, ray.direction, weapons[weapon].bulletDamage);
					}
				}
			}
			// No target was hit.
			else
			{
				...
				DrawShoot(weapons[weapon].gameObject, destination, Vector3.up, null, null, false, false);
			}
			// Play shot sound.
	...
	private void DrawShoot(GameObject weapon, Vector3 destination, Vector3 targetNormal, Transform parent, GameObject parentObject,
		bool placeSparks = true, bool placeBulletHole = true)
	{
		...
		if (placeBulletHole && parentObject.isStatic)
		{
	...
	private void LateUpdate()
	{
		...
		if (behaviourManager.GetAnim.GetBool(reloadBool))
		{
			rightHand.Rotate(ReloadAim);
		}
		else if (isAiming)
		{
			rightHand.Rotate(RightHandAim);
		}
		// Correct right hand position when covering.
		else if ((!isAiming || isAimBlocked)
